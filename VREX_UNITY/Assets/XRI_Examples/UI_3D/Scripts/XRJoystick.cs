...
namespace UnityEngine.XR.Content.Interaction
{
    ...
    {
        private const float k_MaxDeadZonePercent = 0.9f;
        ...
        [Tooltip("Controls how the joystick moves")] [SerializeField]
        private JoystickType m_JoystickMotion = JoystickType.BothCircle;

        [SerializeField] [Tooltip("The object that is visually grabbed and manipulated")]
        private Transform m_Handle = null;

        [SerializeField] [Tooltip("The value of the joystick")]
        private Vector2 m_Value = Vector2.zero;

        [SerializeField] [Tooltip("If true, the joystick will return to center on release")]
        private bool m_RecenterOnRelease = true;

        [SerializeField] [Tooltip("Maximum angle the joystick can move")] [Range(1.0f, 90.0f)]
        private float m_MaxAngle = 60.0f;

        [SerializeField]
        [Tooltip("Minimum amount the joystick must move off the center to register changes")]
        [Range(1.0f, 90.0f)]
        private float m_DeadZoneAngle = 10.0f;

        [SerializeField] [Tooltip("Events to trigger when the joystick's x value changes")]
        private ValueChangeEvent m_OnValueChangeX = new();

        [SerializeField] [Tooltip("Events to trigger when the joystick's y value changes")]
        private ValueChangeEvent m_OnValueChangeY = new();
        ...
        {
            var direction = m_Interactor.GetAttachTransform(this).position - m_Handle.position;
        ...
        private void OnDrawGizmosSelected()
        {
            ...
            if (m_JoystickMotion != JoystickType.LeftRight)
            {
                ...
                var axisPoint1 = angleStartPoint +
                                 transform.TransformDirection(Quaternion.Euler(m_MaxAngle, 0.0f, 0.0f) * Vector3.up) *
                                 k_AngleLength;
                ...
                if (m_DeadZoneAngle > 0.0f)
                {
                    ...
                    axisPoint1 = angleStartPoint +
                                 transform.TransformDirection(
                                     Quaternion.Euler(m_DeadZoneAngle, 0.0f, 0.0f) * Vector3.up) * k_AngleLength;
                    axisPoint2 = angleStartPoint +
                                 transform.TransformDirection(Quaternion.Euler(-m_DeadZoneAngle, 0.0f, 0.0f) *
            ...
            if (m_JoystickMotion != JoystickType.FrontBack)
            {
                Gizmos.color = Color.green;
                var axisPoint1 = angleStartPoint +
                                 transform.TransformDirection(Quaternion.Euler(0.0f, 0.0f, m_MaxAngle) * Vector3.up) *
                                 k_AngleLength;
                var axisPoint2 = angleStartPoint +
                                 transform.TransformDirection(Quaternion.Euler(0.0f, 0.0f, -m_MaxAngle) * Vector3.up) *
                                 k_AngleLength;
                ...
                if (m_DeadZoneAngle > 0.0f)
                {
                    ...
                    axisPoint1 = angleStartPoint +
                                 transform.TransformDirection(
                                     Quaternion.Euler(0.0f, 0.0f, m_DeadZoneAngle) * Vector3.up) * k_AngleLength;
                    axisPoint2 = angleStartPoint +
                                 transform.TransformDirection(Quaternion.Euler(0.0f, 0.0f, -m_DeadZoneAngle) *
                                                              Vector3.up) * k_AngleLength;
                    Gizmos.DrawLine(angleStartPoint, axisPoint1);
                    Gizmos.DrawLine(angleStartPoint, axisPoint2);
                }
