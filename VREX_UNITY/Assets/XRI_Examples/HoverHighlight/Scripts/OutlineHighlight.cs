namespace UnityEngine.XR.Content.Rendering
{
    ...
    {
        private enum OutlineSource
        {
            Shader = 0,
            Material
        }

        private const float k_OutlineWidth = 0.005f;
        private const string k_ShaderColorParameter = "_Color";
        private const string k_ShaderWidthParameter = "g_flOutlineWidth";
        private static readonly int k_GFlOutlineWidth = Shader.PropertyToID(k_ShaderWidthParameter);
        private static readonly int k_Color = Shader.PropertyToID(k_ShaderColorParameter);

#pragma warning disable 649
        [SerializeField] [Tooltip("How the highlight material will be applied to the renderer's material array.")]
        private MaterialHighlightMode m_HighlightMode = MaterialHighlightMode.Replace;

        [SerializeField] [Tooltip("Selects source for the highlight material. Either using a shader or material.")]
        private OutlineSource m_OutlineSource = OutlineSource.Shader;
        ...
        private bool m_StartWithMaterialValues;
#pragma warning restore 649

        private Material m_InstanceOutlineMaterial;

        private bool m_Animating = false;
        private bool m_AnimatingIn = false;
        ...
        /// </summary>
        private float relativeOutlineScale => outlineScale * k_OutlineWidth;
        ...
        /// </summary>
        private float startingRelativeOutlineScale => startingOutlineScale * k_OutlineWidth;
        ...
        public void PlayPulseAnimation(bool pulseUp = true)
        {
            ...
            else
            {
                ...
                if (m_Animating)
                {
                    if (m_AnimatingIn != pulseUp) m_TransitionTimer = 1.0f - m_TransitionTimer;
                }
                else
                {
                    m_Animating = true;
        ...
        private void InstantiateHighlightMaterial()
        {
            ...
            switch (m_OutlineSource)
            {
                case OutlineSource.Material:
                    ...
                    {
                        Debug.LogError(
                            $"{gameObject.name} Outline highlight has no material assigned. Please assign outline material.",
                            this);
                        enabled = false;
                        break;
                    }
                ...
                case OutlineSource.Shader:
                    ...
                    {
                        Debug.LogError(
                            $"{gameObject.name} Outline highlight has no shader assigned. Please assign outline shader. ",
                            this);
                        enabled = false;
                        break;
                    }
                ...
                default:
                    Debug.LogError(
                        $"{gameObject.name} Outline highlight has an invalid highlight mode {m_OutlineSource}.", this);
                    enabled = false;
                    break;
            }
        }
